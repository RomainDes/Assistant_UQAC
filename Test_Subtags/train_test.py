...
#Ouverture du json
with open('intentions_test.json', 'r') as f:
...
for intention in intentions['intentions']:
    tag = intention['tag']
    tags.append(tag)
    for subtag in intention.get('subtags', []):
        subtag_name = subtag['name']
        tags.append(subtag_name)
        for pattern in subtag['patterns']:
            ...
            phraseTokenizee_tag.append((w, subtag_name))
...
for (pattern_sentence, tag) in phraseTokenizee_tag:
    # Créer un dictionnaire pour chaque phrase
    bag = bag_of_words(pattern_sentence, all_words)
    label = tags.index(tag)
    # Ajouter la phrase et l'étiquette correspondante dans la liste
    x_train.append(bag)
    y_train.append(label)
    # Parcourir les sous-tags de l'étiquette actuelle et ajouter leurs phrases avec l'étiquette correspondante
    if tag in intention.get('tag', []):
        for subtag in intention.get('subtags', []):
            subbag = bag_of_words(subtag['patterns'], all_words)
            sublabel = tags.index(subtag['name'])
            x_train.append(subbag)
            y_train.append(sublabel)

# Convertir la liste de dictionnaires en un tableau numpy
